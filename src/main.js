...
carousel.slides.activeSlideIndicators = document.getElementsByClassName(
  'carousel-indicators',
)[0].children;
...
document.addEventListener('DOMContentLoaded', () => {
  if (carousel.options.motionBlur) {
    Array.from(carousel.slides.activeSlide).forEach((element) => {
...
/* Carousel Modules */
const trueModulo = (n, m) => ((m % n) + n) % n;

const indexActiveSlide = function () {
  for (let iterator = 0; iterator < carousel.slides.activeSlide.length; iterator += 1) {
    if (carousel.slides.activeSlide[iterator].classList.contains('show')) {
      return iterator;
    }
  }
};
...
const touchSnap = event => event.touches || event.originalEvent.touches;
...
carousel.controls.leftControl.addEventListener('click', () => {
  ...
  clearTimeout(carousel.slides.autoSlideIntervalContainer);
  slideTimerCall();
...
/* Five star markup */
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('.star-rating > ul').forEach((elm) => {
    for (let i = 0; i < carousel.options.starCount; i += 1) {
      const starAppender = document.createElement('i');
